...
## Function to show the result of a simple model
def simple_model(epochs):
    """
    Input:
        - epochs: epochs object
    ...
    for time in range(len(epochs.times)):
        ...
        # Choose a model with bagging
        model = make_pipeline(preprocessing, BaggingClassifier(SVC(random_state=0), n_estimators=10, n_jobs=-2))
...
## Function to plot the result of a simple model
def plot_simple_model(results, epochs):
    """
    Input:
        - results: list of accuracy scores
        - epochs: epochs object
    ...
    plt.plot(epochs.times, results)
    plt.hlines(1/6, epochs.times[0], epochs.times[-1], linestyle='dashed', color='black')
    plt.xlabel('Time (s)')
...
def effects_of_pca(epochs, param_grid, best_time):
    ...
    for i in range(len(number_of_components)):
        print(i/10 * 100)

        pca = UnsupervisedSpatialFilter(PCA(number_of_components[i]), average=False)
        pca_data = pca.fit_transform(X_all)


        # Select the data
        X = pca_data[:,:,best_time]
        y = y_all[:, 2]
        ...
        # Cross-validate accuracy scores
        results_PCA.append(search.best_score_)

    return results_PCA, number_of_components

## Function to plot the effects of PCA
def plot_effects_of_pca(results_PCA, number_of_components):
    """
    Input:
        - results_PCA: list of accuracy scores with PCA
        - number_of_components: list of number of components
    ...
    plt.plot(number_of_components, results_PCA)
